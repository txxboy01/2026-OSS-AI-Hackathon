...
def test_all_three_demo_samples_use_real_validation(factory):
    samples = json.loads((ROOT / "samples/messages.json").read_text())
    expectations = {
        e["id"]: e for e in json.loads((ROOT / "tests/fixtures/expected_samples.json").read_text())
    }
...
def test_gold_cases_are_synthetic_and_complete():
    cases = json.loads((ROOT / "tests/fixtures/gold_cases.json").read_text())
