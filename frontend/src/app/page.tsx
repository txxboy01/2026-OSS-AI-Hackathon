...
export default function Home() {
  ...
  function reset() { setAnalysis(null); setAnalysisError(""); setShowGuide(false); setSelectedAction(null); setGuidance(null); setGuidanceError(""); }
  ...
  async function runAnalysis() {
    ...
    } catch (error) { setAnalysisError(error instanceof Error ? error.message : "분석 요청을 처리하지 못했습니다."); }
  ...
  return <main>
    ...
    <section className="hero" id="top"><div className="shell"><h1>의심 메시지,<br />먼저 확인하세요.</h1><p className="subhead">AI가 메시지 속 ‘요구’를 원문과 함께 보여드립니다.</p>
      <section className="analyzer" id="analyze">
        {!analysis ? <>
          ...
          <label htmlFor="message">확인할 메시지를 붙여넣으세요</label><textarea id="message" value={message} onChange={(event) => { setMessage(event.target.value); setAnalysisError(""); }} />
          ...
          <div className="privacy"><b>분석의 한계</b><span>{analysis.limitation}</span></div><div className="buttons"><button className="primary" onClick={() => setShowGuide(true)}>내 상황에 맞는 대응 보기 <b>→</b></button><button className="secondary" onClick={reset}>새 메시지 분석</button></div>
    ...
    {modal && <div className="modalBackdrop" role="presentation" onClick={() => setModal(null)}><section className="modal" role="dialog" aria-modal="true" aria-label={modal === "about" ? "서비스 소개" : "개인정보 원칙"} onClick={(event) => event.stopPropagation()}><button className="close" aria-label="닫기" onClick={() => setModal(null)}>×</button>{modal === "about" ? <><p className="eyebrow">HOW RPM WORKS</p><h2>판정이 아니라,<br />멈추고 확인할 근거를 보여줍니다.</h2><p className="modalLead">RPM은 실제 피싱이나 계정 침해를 확정하지 않습니다. 메시지가 요구하는 행동과 원문 근거를 보여주고, 사용자가 안전한 다음 행동을 선택하도록 돕습니다.</p><div className="modalSteps"><Step number="1" title="메시지 입력" text="안전 샘플 또는 비식별 처리한 메시지를 분석합니다." /><Step number="2" title="원문 근거 확인" text="AI 인용문을 원문과 비교합니다." /><Step number="3" title="상황별 대응" text="실제로 한 행동에 맞는 다음 순서를 확인합니다." /></div></> : <><p className="eyebrow">PRIVACY BY DESIGN</p><h2>입력은 최소화하고,<br />분석 결과는 저장하지 않습니다.</h2><p className="modalLead">실제 메시지를 분석할 때도 비밀번호, 인증번호, 계좌번호 등 민감정보를 지운 본문만 입력해야 합니다.</p><ul className="privacyList"><li>비밀번호·인증번호·계좌번호·주민번호는 입력하지 않습니다.</li><li>입력 메시지와 분석 결과를 브라우저 저장소에 보관하지 않습니다.</li><li>새로고침하거나 탭을 닫으면 현재 분석 결과가 사라집니다.</li><li>AI API 키는 프론트엔드에 노출하지 않고 백엔드에서 관리합니다.</li></ul></>}</section></div>}
  </main>;
}
